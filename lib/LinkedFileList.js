...
function LinkedFileList (_path) {
  ...
  this.withPath = (toRun) => {
    const returnDir = process.chdir.bind(null, process.cwd());
    process.chdir(this.pathJoin(''));
    let result;
    try {
      result = toRun();
    } catch (err) {
  ...
  this.initDisk = () => { // UNIT TEST PASSED
    if (fs.readdirSync(this.pathJoin('')).length > 0) {
      throw new Error('Cannot initiate database: directory not empty');
    }
    ...
    this.withPath(() => {
      ...
      child_process.execSync('git remote add origin PLACEHOLDER_URL').toString();
    });
  };
  ...
  */
  ...
  this.commit = (message) => { // UNIT TEST PASSED
    return this.withPath(() => {
      let output = child_process.execSync('git add -A').toString() + '\n';
      try {
        output += child_process.execSync(`git commit -m "${shell.quote([message])}"`).toString();
      } catch (_) {
        output += child_process.execSync('git status').toString();
      }
      return output;
    });
  };

  this.gitConfig = (command) => {
    return this.withPath(() => {
      let output = child_process.execSync(
        'git config --local ' + shell.quote(shell.parse(command))
      ).toString();
      return output;
    });
  };
  ...
  this.push = () => {
    return this.withPath(() => {
      let output = '';
      try {
        output += child_process.execSync('git push').toString();
      } catch (_) {
        output += 'git push failed. Check Termux console.';
      }
      return output;
    });
  };
